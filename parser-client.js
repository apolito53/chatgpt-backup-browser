...
async function parseConversationsLocally(rawText) {
  ...
  postLocalProgress("Summarizing conversations...", 45);
  const conversations = [];
  ...
  for (let index = 0; index < rawData.length; index += 1) {
    ...
    if (index > 0 && index % 100 === 0) {
      ...
      postLocalProgress(
        `Summarizing conversations locally... ${index.toLocaleString()} / ${rawData.length.toLocaleString()}`,
        progress,
      );
...
function parseConversationsInWorker(rawText) {
  if (window.location.protocol === "file:") {
    return parseConversationsLocally(rawText);
  }

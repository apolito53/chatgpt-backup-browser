...
(() => {
    ...
    function getSelectedConversationSourceFile() {
        if (state.attachedFolderFiles.length) {
            return state.attachedFolderFiles.find((file) => (file.webkitRelativePath || file.name).endsWith("conversations.json")) || state.attachedFolderFiles.find((file) => (file.webkitRelativePath || file.name).endsWith("chat.html")) || null;
        }
        if (elements.fileInput.files?.length) {
            return elements.fileInput.files[0];
        }
        if (elements.folderInput.files?.length) {
            return Array.from(elements.folderInput.files).find((file) => (file.webkitRelativePath || file.name).endsWith("conversations.json")) || Array.from(elements.folderInput.files).find((file) => (file.webkitRelativePath || file.name).endsWith("chat.html")) || null;
        }
        return null;
    }
    ...
    async function parseConversationsLightweight(rawText) {
        ...
        for (let index = 0; index < rawData.length; index += 1) {
            ...
            if (index > 0 && index % 25 === 0) {
                ...
                postLocalProgress(`Summarizing conversations in lightweight mode... ${index.toLocaleString()} / ${rawData.length.toLocaleString()}`, progress);
...
(${(() => {
            self.addEventListener("message", (event) => {
                ...
                const namespace = (self.ChatBrowser ||= {});

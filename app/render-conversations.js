...
(() => {
    ...
    function ensureSelectedConversationPage() {
        const selectedIndex = state.filteredConversations.findIndex((conversation) => conversation.id === state.selectedConversationId);
        if (selectedIndex === -1) {
    ...
    function searchMatchesConversation(conversation, query, role) {
        ...
        return conversation.messages.some((message) => isReaderVisibleMessage(message) && message.role === role && message.text.toLowerCase().includes(lowerQuery));
    }
    ...
    function messageNeedsDetailHydration(message) {
        ...
        if (Object.prototype.hasOwnProperty.call(message, "rawContent")
    ...
    function replaceConversationRecord(nextConversation) {
        ...
        state.index.conversations = state.index.conversations.map((conversation) => (conversation.id === nextConversation.id ? nextConversation : conversation));
        state.filteredConversations = state.filteredConversations.map((conversation) => (conversation.id === nextConversation.id ? nextConversation : conversation));
    }
    async function loadSelectedConversationDetails() {
        const conversation = state.index?.conversations.find((item) => item.id === state.selectedConversationId);
        ...
        try {
            ...
            conversationDetailErrors.set(conversation.id, error instanceof Error ? error.message : "Failed to load full conversation details.");
        }
        finally {
    ...
    function moveConversationSelection(direction) {
        ...
        const currentIndex = state.filteredConversations.findIndex((conversation) => conversation.id === state.selectedConversationId);
        if (currentIndex === -1) {
    ...
    function renderConversationsView() {
      ...
      `;
        ...
        updateConversationListPager();
        if (state.pageType === "conversation") {
            renderConversation(state.filteredConversations.find((conversation) => conversation.id === state.selectedConversationId) || null);
        }
        else {
            renderConversation(null);
        }

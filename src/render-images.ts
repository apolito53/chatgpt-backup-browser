(() => {
  ...
  function compareImages(a: ImageRecord, b: ImageRecord, mode: string): number {
    switch (mode) {
      case "created-desc":
      case "updated-desc":
  ...
  function searchMatchesImage(image: ImageRecord, query: string): boolean {
    if (!query) {
      return true;
    }
  ...
  function renderImageDetail(image: ImageRecord | null): void {
    if (!image) {
  ...
  function renderImagesView(): void {

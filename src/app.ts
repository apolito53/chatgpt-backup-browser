(() => {
  ...
  function updateFolderDigestButton(): void {
    const hasFolderSelection = Boolean(
      (elements.folderInput.files && elements.folderInput.files.length)
  ...
  async function collectFilesFromDirectoryHandle(handle: any, prefix = handle.name): Promise<File[]> {
    const files: File[] = [];
  ...
  async function hasDirectoryPermission(handle: any): Promise<boolean> {
  ...
  async function chooseFolderWithDirectoryAccess(): Promise<void> {
    if (!browserSupportsDirectoryAccess()) {
      elements.folderInput.click();
      return;
    }
  ...
  function getFolderSessionInfo(fileList: FileList | File[] | null | undefined): {
    files: File[];
  ...
  function shouldHydrateLightweightImages(): boolean {
    return Boolean(
  ...
  async function attachImagesToCurrentFolderSession(fileList: FileList | File[]): Promise<boolean> {
  ...
  async function ensureImagesReadyForLightweightMode(): Promise<boolean> {
  ...
  function formatRecentArchiveTimestamp(timestamp: number): string {
  ...
  function buildRestoreStatusMessage(sessionRecord: SessionRecordSummary): string {
  ...
  function applyStoredSession(sessionRecord: SessionRecord): void {
  ...
  async function confirmArchiveReplacement(nextSourceLabel: string): Promise<boolean> {
    if (!hasLoadedArchive()) {
      return true;
    }
  ...
  async function parseFolder(fileList: FileList | File[], options: { forceImages?: boolean } = {}): Promise<void> {
  ...
  async function restoreFromPickerOrCache(): Promise<void> {
  ...
  elements.recentArchivesList.addEventListener("click", async (event) => {
    const button = (event.target as HTMLElement).closest<HTMLElement>("[data-session-key]");
    if (!button) {
      return;
    }
  ...
  elements.listPageSizeTop.addEventListener("change", (event) => {
    setConversationListPageSize((event.target as HTMLSelectElement).value);
  });
  ...
  elements.parserModeSelect.addEventListener("change", (event) => {
    state.parserMode = (event.target as HTMLSelectElement).value === "lightweight" ? "lightweight" : "robust";
    saveUiState();
    updateFolderDigestButton();
  });
  ...
  elements.statusBanner.addEventListener("keydown", (event) => {
    if (
      elements.statusBanner.classList.contains("action")

(() => {
  ...
  function extractLeadingJsonValue(sourceText: string): string {
  ...
  function extractJsonPayload(htmlText: string): string {
  ...
  function extractConversationArray(rawText: string): string {
  ...
  function coerceTextParts(content: any): string {
    if (!content) {
      return "";
    }
  ...
  function getConversationModelInfo(conversation: any): { modelSlug: string; defaultModelSlug: string } {
  ...
  function lineageForConversation(conversation: any): string[] {
  ...
  function summarizeConversation(conversation: any, index: number): ConversationRecord {
  ...
  function summarizeConversationLightweight(conversation: any, index: number): ConversationRecord {
    const orderedIds = lineageForConversation(conversation);
    const conversationId = conversation.conversation_id || conversation.id || `conversation-${index}`;
    const messages: MessageRecord[] = [];
    const conversationModelInfo = getConversationModelInfo(conversation);
